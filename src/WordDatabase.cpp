...
static
bool loadZipData(const std::string& path, std::vector<char>& to) {
	Gzip gz;
	if(!gz.open) {
...
static
bool loadNormalData(const std::string& path, std::vector<char>& to) {
...
bool loadFileData(const std::string& path, std::vector<char>& to) {
	if(!loadZipData(path + ".gz", to)) {
		if(!loadNormalData(path + ".xml", to)) {
